...
Run_Hubs_Model <- function(runCount, swpGraph, randGraph, hubThreshold, 
 ...
 # Returns a list of the vertex number of all the hubs. 
  for(step in seq(from=1, to=timeSteps, by=1)){
    ...
    # rand hub-hub connections
    randHubHub  <- HubHub(randGraph, randHubInd)
    randHubHub1 <- randHubHub$hubhub1
    randHubHub2 <- randHubHub$hubhub2

print(paste('step: ', step))
print(length(swpHubInd))
print(degree(swpGraph)[swpHubInd])
print(swpHubInd)
print(hub.score(swpGraph)$vector[swpHubInd])
...
print(intersect(swpNonHubs,nonAdjZ))
print("---")
...
print("rand else if")
        ...
        nonAdjZR <- which(!(1:vcount(randGraph) %in% unlist(get.adjlist(randGraph)[xR])))
        zR  <- sample( intersect(randNonHubs,nonAdjZ) , 1 )
print(randNonHubs)
print(zR)
print(nonAdjZ)
print(randGraph[xR,zR])
...
print(randGraph[xR,zR])
      ...
      # If there are hubs, but no hub-hub connections
        ...
        if( length(randHubInd) == 1){
          print("hub1R")
          xR <- randHubInd[1]       # Selects the Hub
          yR <- sample(unlist(get.adjlist(randGraph)[xR]), 1) # Random xR-adj
          randGraph[xR,yR] <- FALSE # Remvoes edge between x and y
print(randHubInd)
print(xR)
          nonAdjZR <- which(!(1:vcount(randGraph) %in% unlist(get.adjlist(randGraph)[xR])))
          zR <- sample(intersect(randNonHubs, nonAdjZR), 1) 
          randGraph[xR,zR] <- 1     # Adds new edge between xR and zR
        }else{ # If there is more than 1 hub.
          ...
          nonAdjZR <- which(!(1:vcount(randGraph) %in% unlist(get.adjlist(randGraph)[x])))
          zR  <- sample( intersect(randNonHubs,nonAdjZ) , 1 ) 
          randGraph[xR,zR] <- 1            # Adds edge between x and z
          }
        }
